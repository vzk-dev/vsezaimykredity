...
SeoTabs.prototype.bindEvents = function () {
    ...
    $(this.options.trigger).on('click', function (e) {
        ...
        if (!self.currentTab.data('loaded')) {
            self.load(self.currentTab.data('id'));
        } else {
            self.setMeta();
        }
...
SeoTabs.prototype.switchToTab = function (id) {
    var tab = $(this.options.trigger + '[data-id="' + id + '"]').eq(0);
    if (!tab.length) return;
    this.currentTab = tab;
    this.currentTab.tab('show');
};

SeoTabs.prototype.load = function (id, callback) {
    var self = this,
        params = {
            id: id,
    ...
    $.ajax({
        ...
        success: function (r) {
            if (r.success) {
                if(r.object.seo) {
                    self.getContentElement().html(r.message);
                }
                self.setMeta(r.object.meta);
            } else {
