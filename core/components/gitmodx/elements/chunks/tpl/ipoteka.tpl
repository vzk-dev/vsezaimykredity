<div class="card-offer-bg mt-4">
        ...
        <div class="col-12 col-md-3 col-lg-2">
            {if '!pdoField' | snippet : [ 'id' => $id, 'field' => 'img', 'top'=>2]}
                <div class="card-offer__image">
                    <img class="" src="{'!pdoField' | snippet : [ 'id' => $id, 'field' => 'img', 'top'=>2]}" alt="{'!pdoField' | snippet : [ 'id' => $id, 'field' => 'pagetitle', 'top'=>2]}">
                </div>
            {/if}
            <div class="mt-2 mb-2">
        ...
        <div class="col-12 col-md-9 col-lg-10">
            ...
            <div class="row">
                ...
                <div class="col-6 col-md-4 col-lg-3 mb-2">
                    ...
                    <div class="card-offer-value card-offer-value--color">{$_modx->runSnippet('!get_range_value', ['value'=>$_pls["tv.ipoteka_summ"],'get_max'=>1]) | number : 0 : '.' : ' '} руб.</div>
                </div>
                {/if}
                {if $_pls["tv.ipoteka_age"]}
                <div class="col-6 col-md-3 col-lg-2 mb-2">
                    <div class="card-offer-title">Возраст от</div>
                    <div class="card-offer-value">{$_modx->runSnippet('!get_range_value', ['value'=>$_pls["tv.ipoteka_age"]]) | decl:'года|лет':true}</div>
                </div>
                {/if}
                {if $_pls["tv.ipoteka_time"]}
                <div class="col-6 col-md-3 col-lg-2 mb-2">
                    <div class="card-offer-title">Срок до</div>
                    <div class="card-offer-value">{$_modx->runSnippet('!get_range_value', ['value'=>$_pls["tv.ipoteka_time"],'get_max'=>1])} мес.</div>
                </div>
                {/if}
                {if $_pls["tv.ipoteka_percent"]}
                <div class="col-6 col-md-3 col-lg-2 mb-2">
                    <div class="card-offer-title">Процент</div>
                    <div class="card-offer-value">{$_modx->runSnippet('!get_range_value', ['value'=>$_pls["tv.ipoteka_percent"],'get_min'=>0])}%</div>
                </div>
                {/if}
                <div class="col col-md-6 col-lg-3 mb-2">
                    {if $_pls["tv.referal_link"]}
                    <a href="{$_pls['tv.referal_link']}" onclick="yaCounter70630705.reachGoal('{$_pls["tv.target"]}'); return true;" class="btn btn-card">Оформить заявку</a>
                {else}
                    {if $_pls["tv.link_to_site"]}
                        <a href="https://{$_pls['tv.link_to_site']}" onclick="yaCounter70630705.reachGoal('link'); return true;" class="btn btn-card">Оформить заявку</a>
                    {/if}
                {/if}
                </div>
            </div>
            <div class="row mt-lg-2 mt-md-0 mt-2">
                ...
                {elseif $_pls["tv.probability_of_approval"] >= 51 && $_pls["tv.probability_of_approval"] <= 79}
                <div class="col-md-6 col-lg-6">
                    <div class="card-offer-text">Процент одобрения заявок по статистике сайта
                    </div>
                    <div class="card-offer-approval"> {*другие цвета: --color-yellow и --color-green *}
    ...
    <div class="card-offer-details" style="display:none;">
        <div class="row card-line">
            {if $_pls['tv.ipoteka_content']}
                {set $rows = $_pls['tv.ipoteka_content']}
                {foreach $rows as $row}
                    {if $row.active == 1}
                    ...
                    <div class="row mb-4">
                        <div class="col-md-4 col-lg-4">
                            <div class="calc-input">
                                ...
                                <input class="ipoteka_calc_summ calc-input-value" type="text" value="3000000">
                            </div>
                        </div>
                        <div class="col-md-4 col-lg-4">
                            <div class="calc-input">
                                <label class="calc-title">Первоначальный взнос</label>
                                <input class="ipoteka_calc_summ_realty_fee calc-input-value" type="text" value="500000">
                            </div>
                        </div>
                        <div class="col-md-4 col-lg-4">
                            <div class="calc-input">
                                <label class="calc-title">Срок</label>
                                <input class="ipoteka_calc_time calc-input-value" type="text" value="{$_modx->runSnippet('!get_range_value', ['value'=>$_pls["tv.ipoteka_time"],'get_max'=>1])}">
                                <input style="display:none;" class="ipoteka_calc_rate calc-input-value" type="text" value="{$_modx->runSnippet('!get_range_value', ['value'=>$_pls["tv.ipoteka_percent"],'get_min'=>0])}">
                            </div>
                        </div>
                    </div>
                    <div class="row mb-2">
        ...
        {*<div class="row mt-4">
            ...
            <div class="col-12 col-md-6 col-lg-3 mb-3">
                <div class="rating">
                    <a href="{$id | url}" class="btn btn-rating">Подробнее</a>
                </div>
            </div>
        </div>
