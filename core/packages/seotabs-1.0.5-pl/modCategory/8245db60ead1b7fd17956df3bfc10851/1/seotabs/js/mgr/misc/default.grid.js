SeoTabs.grid.Default = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        baseParams: {},
        cls: config['cls'] || 'main-wrapper seotabs-grid',
        autoHeight: true,
        paging: true,
        remoteSort: true,
        autosave: true,
        ...
        viewConfig: {
            ...
            getRowClass: function (rec) {
                ...
                if (rec.data['published'] != undefined && rec.data['published'] == 0) {
                    cls.push('seotabs-row-unpublished');
                }
                if (rec.data['active'] != undefined && rec.data['active'] == 0) {
                    cls.push('seotabs-row-inactive');
                }
                if (rec.data['deleted'] != undefined && rec.data['deleted'] == 1) {
                    cls.push('seotabs-row-deleted');
                }
                if (rec.data['required'] != undefined && rec.data['required'] == 1) {
                    cls.push('seotabs-row-required');
                }
                return cls.join(' ');
            }
        },
    });
    SeoTabs.grid.Default.superclass.constructor.call(this, config);
...
Ext.extend(SeoTabs.grid.Default, MODx.grid.Grid, {
    ...
    getColumns: function (config) {
        return [{
            ...
            header: _('seotabs.header_actions'),
            dataIndex: 'actions',
    ...
    getMenu: function (grid, rowIndex) {
        ...
        var menu = SeoTabs.utils.getMenu(row.data['actions'], this, ids);
...
Ext.reg('seotabs-grid-default', SeoTabs.grid.Default);