...
switch ($action) {
    case 'filter':
        ...
        if (!empty($_REQUEST['parents'])) {
            $scriptProperties['parents'] = $paginatorProperties['parents'] = $_REQUEST['parents'];
        }
        
        if (!empty($_REQUEST['groupby'])) {
            $config['paginatorProperties']['groupby'] = $paginatorProperties['groupby'] = $_REQUEST['groupby'];
            $config['scriptProperties']['groupby'] = $scriptProperties['groupby'] = $_REQUEST['groupby'];
        }
        
        $config['paginatorProperties']['depth'] = $paginatorProperties['depth'] = 0;
        $config['scriptProperties']['depth'] = $scriptProperties['depth'] = 0;
        ...
        $response = array(
            'success' => true,
            'message' => [$config,$paginatorProperties,$scriptProperties],
            'data' => array(
                'request' => $_REQUEST,
