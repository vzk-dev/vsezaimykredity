...
class SeoTabsTools
{
    ...
    public function getTabDataByAlias($rid, $alias, array $options = array())
    {
        ...
        $columnPrefix = $this->modx->getOption('columnPrefix', $options, '', true);
        $onlySeo = $this->modx->getOption('onlySeo', $options, false, true);
        $result = $this->getCache($cacheKey);
        if ($result === null) {
            $result = array();
            $q = $this->modx->newQuery($classKey);
     ...
     * @return string|null
     */
    public function getContentTypeExtension($mimeType = 'text/html')
    {
        $cacheKey = md5('getContentTypeExtension' . $mimeType);
        $result = $this->getCache($cacheKey);
        if ($result === null) {
            if ($contentType = $this->modx->getObject('modContentType', array('mime_type' => $mimeType))) {
                $result = $contentType->get('file_extensions');
                $this->setCache($cacheKey, $result);
            }
        }
        return $result;
