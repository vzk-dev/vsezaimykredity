...
if ($transport->xpdo) {
    ...
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            $modelPath = $modx->getOption('seotemplates_core_path', null,
                    $modx->getOption('core_path') . 'components/seotemplates/') . 'model/';
            $modx->addPackage('seotemplates', $modelPath);

            $manager = $modx->getManager();
            $objects = array();
            $schemaFile = MODX_CORE_PATH . 'components/seotemplates/model/schema/seotemplates.mysql.schema.xml';
